...
// 컴포넌트: 리액트의 구성요소(UI)
// Props: 컴포넌트의 속성(Properties)의 약어
//! Props 전달
...
function MultiProps( { name = '김명진', colorProps }: MultiPropsType) {
  ...
  // : 위처럼 객체나 배열 내부의 요소들을 한번 각각의 요소 변수에 할당하는
  //  , 구조 분해 할당(비구조화 할당) 가능
...
function F_Props() {
  ...
  return (
    <div>
      {/* 
      Props는 부모 컴포넌트 내에서 자식 컴포넌트에서 '속성'으로 값을 전달
      >> 전달된 데이터는 컴포넌트의 '매개변수'로 받아 사용

      !! props의 경우 자식 컴포넌트에 키: 값 쌍으로(객체 형식) 데이터를 전달
      ...
      */}
      ...
      <MultiProps {...props}/> 
      {/*...props가 구조 분해 할당을 사용하여 각각의 속성명을 찾아감(순서가 상관없음) */}
