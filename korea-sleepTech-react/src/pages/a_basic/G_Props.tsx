...
export const Wrapper = ({ children }: ChildrenType) => {
  return(
    <div style={{border: '2px solid black', padding: '16px', backgroundColor: 'lightgreen'}}>
      {/* props로 전달받은 UI를 HTML 내에서 출력 시 JS의 문법 출력처럼 사용 */}
      {children}
    </div>
  )
}
...
function G_Props() {
  const userData = {
    name: '이승아', age: 30, email: '123qwe'
  }

  return (
    // ReactNode를 props로 전달받는 컴포넌트는 열리고 닫히는 태그내에서 : 전달할 
    <Wrapper>
      <UserCard user={{name: '이승아', age: 30, email: '123qwe'}} />
