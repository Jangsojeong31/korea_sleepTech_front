...
/*
  ! === 부수 효과 (side effect)===
  : 컴포넌트의 주요 기능(상태 관리, UI 렌더링) 발생하는 작업
  ex) API 호출, 이벤트 리스너 등록, 수동 DOM 조작 등

  ! useEffect
  : React 함수형 컴포넌트에서 부수효과를 수행하기 위한 Hook
  - 데이터 가져오기, 컴포넌트 렌더링 시 특정 작업 수행 등에 사용

  ? React의 컴포넌트의 '라이프 사이클'
    1) 마운팅: 컴포넌트가 DOM에 처음 삽입 될 때 실행
    2) 업데이트: 컴포넌트의 재렌더링 (주로 state, props의 변경에 의해 발생)
    3) 언마운팅: 컴포넌트가 DOM에서 제거될 때
  ...
  1) 콜백함수: 주로 화살표 함수
    - 부수효과를 수행하는 함수
  2) 의존성 배열: deps
    - 해당 배열 값이 변경될 때마다 부수효과가 다시 실행
    cf) 빈 배열 - 컴포넌트가 마운트될 때 한번만 실행
      +) 의존성 배열에 값이 있더라도 마운트 시에는 반드시 실행됨
  ...
  구조: useEffect(() => {
          ...
          return () => {}
          ? useEffect의 '정리 함수'
          : useEffect에서 함수를 return하면 해당 함수는 컴포넌트가 화면에서 사라질 떄 (언마운트) 실행
...
function UseEffect01() {
  ...
  useEffect(() => {
    // 빈 배열 - 마운트 시에만 실행 (상태 변화에 반응하지 않는다.)
  ...
  return (
    <div>
      ...
      <span>{name}</span>
      {/* 
      해당 버튼의 클릭으로 컴포넌트 내의 상태 변화가 일어나지만
      : useEffect의 의존성 배열에 count 만 있는 부수효과는 name의 변경에 반응하지 않는다. 
      - count 값 변경에만 반응
      */}
