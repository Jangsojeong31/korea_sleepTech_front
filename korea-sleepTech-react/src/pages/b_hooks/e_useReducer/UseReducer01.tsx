...
// 구조: const [state, dispatch] = useReducer(reducer, initialValue);
// 배열 반환, 2개의 인자

// - state: 현재 상태 값(관리되는 데이터)
// - dispatch: 액션을 발생시켜 상태를 업데이트하는 함수
//            >> 해당 함수에 액션을 전달할 경우 reducer 함수가 호출되어 새로운 상태를 계산
// - reducer: 리듀서 함수 (상태를 어떻게 변경할지 정의한 함수)
//  >> useReducer에 인자로 전달되는 함수 -> 콟백함수
...
export function reducer(state: CountState, action: CountAction): CountState {
  switch(action.type){
    case 'increment':
      return { count : state.count + 1 }; // CountState 타입으로 반환
    case 'decrement':
...
export default function UseReducer01() {
  ...
  // 초기값 {count: 0}은 객체 => state의 타입은 객체
