...
addButton.addEventListener('click', () => {
  // input에 입력값이 있는 경우에만 추가
  if(todoInput.value !== '') {
    // 동적으로 li 태그 생성
    // document.createElement('태그명')
    // - 생성된 태그가 곧바로 웹 문서에 할당되지 않는다.
    // - 보여질 위치의 부모요소에 삽입해야만 보여짐
    let newItem = document.createElement('li');
    // input에 입력된 값을 새로운 li에 
    newItem.textContent = todoInput.value;
    ...
    // ? input 등과 같이 상호작용된 데이터는 사용하고나면 데이터를 초기화
    todoInput.value = '';
  }
});
...
todoList.addEventListener('click', (e) => {
  // 각 할 일 리스트 클릭시 
  // -> 완료한 경우: .completed 속성 추가 & css 디자인 첨부

  // e.target: 실제 이벤트가 발생되는 요소 
  // +) DOM 요소의 태그명 가져오기: HTML요소.tagName
  // 태그명의 알파멧이 모두 대문자 표기
