...
bgButton.onmouseout = function() { // bgButton에 onmouseout이 발생하면 function 실행
...
window.onkeydown = function() { // 
  keydownSpan.textContent = '안녕하세요';
}

// & 2) 인라인 이벤트 핸들러
// : HTML 태그에 직접 onclick, onkeydown 등의 이벤트 속성을 사용하여 함수 지정
// - 사용하지 않는 것 권장 (유지보수와 코드 파싱에 어려움)
...
// & cf) JS 내에서의 HTML 인식
// : JS '객체'로 인식
// - DOM (Doucment Object Model)으로 인식

// body > div > p > span 태그

// 1. document.querySelector('선택자')
// : 같은 선택자가 여러 개일 경우 첫번째 요소만 가져옴
...
divs.forEach(div => div.onclick = function() {
  // 이 function은 div(객체)가 호출한 것 => 함수 내 this는 div
  // 클릭 이벤트가 일어난 요소 그 자체
...
// addEventListener()의 인자: 이벤트, 콜백함수
