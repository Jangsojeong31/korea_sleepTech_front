...
// export const tmp = '';
{
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 3:
                    ...
                    return [2 /*return*/, []]; // 함수의 반환타입이 void가 아니기 때문에 return 지정
    ...
    // ! 5) 사용자 정보 배열을 받아 화면에 표시하는 함수(creatUserCard에 각 객체 전달)
    // @Param) users: UsersType 타입
    // @Return) x(void)
    var displayUsers_1 = function (users) {
        var userList = document.getElementById('user-list');
        // userList: HTMLElement | null
        if (userList) { // userList가 null이 아니라면(HTMLElement 라면)
            userList.innerHTML = '';
    ...
    // @Return: UsersType 타입 (필터링된 사용자들의 데이터)
    ...
    //         , key - 'literal 타입' (정렬 키워드를 저장) - option의 value 속성을 리터럴 타입으로 지정
    ...
    var sortUsers_1 = function (users, key) {
        ...
        /*
        ? [...users]
          : 새로운 배열 주소값!에 기존의 사용자 데이터(요소)만 복사해서 가져옴 - 깊은 복사
          - 배열의 불변성을 보완
        ...
        ? 배열.sort()
          ...
          - cf) 현재 a, b 데이터는 객체 (IUser 인터페이스 타입)
                key가 'name'으로 전달되는 경우 => a[name].localeCompare(b[name]) : 각 데이터들의 name 값을 비교
    ...
    var addEventListener_1 = function (users) {
        ...
        searchInput.addEventListener('input', function () {
            ...
            // 필터링된 데이터를 정렬
            // const sortKey = sortSelect.value; // .value -> string으로 반환
            var sortKey = sortSelect.value; // 타입 단언 -> key는 name 또는 email 타입이기 때문에
