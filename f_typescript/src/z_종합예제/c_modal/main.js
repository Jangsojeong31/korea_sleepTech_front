...
var _this = this;
{
    ...
    var creatUserCard_1 = function (user) {
    ...
    // ! 5) userCard 생성해서 userList에 추가
    var displayUsers_1 = function (users) {
        var userList = document.getElementById('user-list');
        if (userList) {
            userList.innerHTML = '';
            users.forEach(function (user) {
                var userCard = creatUserCard_1(user);
    ...
    var addEventListener_1 = function (users) {
        ...
        if (userList) {
            userList.addEventListener('click', function (e) {
                ...
                if (userCard) {
                    var userId_1 = parseInt(userCard.dataset.userId || '0', 10); // 10진수 정수로 변환
                    var user = users.find(function (u) { return u.id === userId_1; }); // users 배열에서 userId와 일치하는 사용자 객체 반환
                    if (user) {
                        showModal_1(user);
                    }
                }
