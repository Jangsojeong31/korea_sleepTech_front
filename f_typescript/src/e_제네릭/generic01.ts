...
// 2. 제네릭의 필요성
// - 코드 중복 줄임, 타입 안정성 보장('컴파일'시점에 타입을 체크)

// 3. 제네릭 기본 문법
// : 사용할 컴포넌트(변수, 함수, 클래스 등)의 이름 뒤에 <> 안에 '타입 변수'를 지정
...
console.log(generic<string>('안녕')); // 안녕

// +) 하나의 컴포넌트에 여러개의 타입 변수 지정
function pair<T, U>(first: T, second: U): [T, U] {
  return [first, second];
}

let pairOutput = pair<string, number>('hello', 123);
let pairOutput2 = pair<number, string>(345, 'hi');

console.log(pairOutput); // [ 'hello', 123 ]

// cf) 타입 단언과 제네릭 타입을 활용
function pair2<T, U>(first: T, second: T): U {
  return [first, second] as U; // U의 타입을 T[]로 단언
}
