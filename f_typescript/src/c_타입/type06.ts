...
let str1 = '안녕하세요'; // 타입: 문자열
str1 = 'hello';

const str2 = '반갑습니다.'; // 타입: '반갑습니다.' - 리터럴 타입
...
let num2: 123 = 123; // 타입: 123 - 리터럴 타입 
// num2 = 345;

// let bool: true = false; // 타입: true - 리터럴 타입
...
type mixedType2 = [1, 3] | {id: string; password: string;};
...
greet(p1); // 오류발생 x => Person과 구조가 같이 때문에 Person 취급
greet(p2); // 구조적 타이핑에 의해 Person 취급: hobby 속성은 무시
// greet(p3); // age 속성이 없기 때문에 Person이 아님(Person과 속성의 구조가 다름)
...
// user2.address.city = '대전' // 읽기 전용
user2.address.zipCode = '중앙대로';
...
let userData: UserDataType = {
  ...
  age: 29,
  isStudent: false,
  // hobby: ['exercise', 'reading'] // 오류: 필요한 형식은 이 인덱스 시그니처에서 가져옵니다.
}

userData.email = '123345';
