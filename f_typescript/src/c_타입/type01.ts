export const tmp = ''; // 모듈로 정의하기

// ! == 타입스크립트의 '타입' == //
// : 타입 명시(작성)가 필수는 아님
// - 프로그램의 안정성과 가독성을 높이기 위해
// - 타입 명시의 이유: 타입 체킹이 강화

// ? 타입 명시 방법
// : 타입 어노테이션(type annotation: 타입 주석)
// 변수명 뒤에 :(콜론)을 사용하여 타입 지정

// ? 타입 종류
// ? : 기본(string, number, boolea 등), 배열(list, array), void, any, never
...
let anyData = '문자열'; // 타입 생략 가능: 타입 체킹은 생략 x 
// 타입 어노테이션은 생략 가능하지만, 초기화 데이터로 타입이 지정됨

// anyData = 123; // 'number' 형식은 'string' 형식에 할당할 수 없습니다.
...
// - TS는 각각의 타입을 지정된 변수에는 해당 타입만 할당 가능 >> 어노테이션을 사용한 타입 강화 시

let nullType: null;
// nullType = 3;
// '3' 형식은 'null' 형식에 할당할 수 없습니다.ts(2322)
// let nullType: null
nullType = null; // null 값만 할당 가능
// cf) undefined도 동일

// & 5. any 타입 (모든)
// : 모든 타입에 대해 허용하는 타입
// - 타입 검사 오류 방지를 위해 사용 (모든 타입과 호환 가능)
// - 권장 x: 사전에 오류 타입 계산을 할 수 없다(타입스크립트의 이점 활용 불가)
...
// 직접적인 타입 명시되지 않은 경우, 값이 할당되지 전까지는 any 타입으로 인식됨
let unknownData; 
